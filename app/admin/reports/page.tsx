...
export default function ReportsPage() {
  ...
  function exportPDF(){
    ...
    if(tab==="company"){
      fetch(B+"/api/admin/balance",{headers:{Authorization:"Bearer "+token}})
        .then(r=>r.json()).then(logs=>{
          ...
          generateCompanyReportPDF({loans:data.loans||[],financialLogs:arr,period:periodLabel(),generatedAt:new Date().toLocaleString(),companyBalance:bal});
        });
    } else {
      generateLoansReportPDF({loans:filteredLoans,period:periodLabel(),generatedAt:new Date().toLocaleString()});
    }
  }
