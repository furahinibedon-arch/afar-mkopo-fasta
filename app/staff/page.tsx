...
export default function StaffPortal(){
  ...
  return(<Layout portal="staff">
    ...
    {loading?<div className="flex items-center justify-center py-20"><div className="w-8 h-8 rounded-full border-4 border-brand-500 border-t-transparent animate-spin"/></div>:<>
      <div className="grid grid-cols-3 gap-4 mb-8">{[{l:"Pending",v:pending.length,c:"border-amber-400"},{l:"Reviewed",v:reviewed.length,c:"border-emerald-400"},{l:"Total",v:loans.length,c:"border-blue-400"}].map(({l,v,c})=><div key={l} className={`card border-l-4 ${c}`}><p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{l}</p><p className="text-3xl font-black text-navy-800 mt-1">{v}</p></div>)}</div>
      <div className="card mb-6"><h2 className="text-lg font-black text-navy-800 mb-4">Pending Review ({pending.length})</h2>{pending.length===0?<p className="text-slate-400 text-center py-8">No pending applications.</p>:<div className="space-y-4">{pending.map((loan:any)=><LoanCard key={loan.id} loan={loan} badge={badge} busy={busy===loan.id} note={notes[loan.id]||""} onNote={(v)=>setNotes(n=>({...n,[loan.id]:v}))} onApprove={()=>action(loan.id,"APPROVED")} onReject={()=>action(loan.id,"REJECTED")} onPrint={()=>handlePrint(loan)}/>)}</div>}</div>
      <div className="card"><h2 className="text-lg font-black text-navy-800 mb-4">Reviewed ({reviewed.length})</h2>{reviewed.length===0?<p className="text-slate-400 text-center py-8">None yet.</p>:<div className="space-y-3">{reviewed.map((loan:any)=><div key={loan.id} className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0"><div><p className="font-semibold text-navy-800">{loan.borrower?.firstName} {loan.borrower?.lastName}</p><p className="text-xs text-slate-400">Tsh {Number(loan.amount).toLocaleString()}</p></div><div className="flex items-center gap-2">{badge(loan.status)}<button onClick={()=>handlePrint(loan)} className="btn-secondary text-xs py-1 px-2">Print</button></div></div>)}</div>}</div>
    </>}
  </Layout>);
}
function LoanCard({loan,badge,busy,note,onNote,onApprove,onReject,onPrint}:any){
  return(<div className="card-hover border border-slate-200"><div className="flex flex-col sm:flex-row gap-4 sm:items-start sm:justify-between"><div className="flex-1"><div className="flex items-center gap-2 mb-2"><span className="font-black text-navy-800">{loan.borrower?.firstName} {loan.borrower?.lastName}</span><span className="badge-pending">PENDING</span></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm text-slate-600"><div><p className="label">Amount</p><p className="font-bold text-navy-800">Tsh {Number(loan.amount).toLocaleString()}</p></div><div><p className="label">Interest</p><p className="font-bold text-navy-800">{Number(loan.interestRate)}%</p></div><div><p className="label">Period</p><p className="font-bold text-navy-800">{loan.repaymentPeriod} days</p></div><div><p className="label">Applied</p><p className="font-bold text-navy-800">{new Date(loan.createdAt).toLocaleDateString()}</p></div></div>{loan.purpose&&<p className="mt-2 text-sm text-slate-500 italic">"{loan.purpose}"</p>}</div><div className="flex flex-col gap-2 min-w-[180px]"><textarea value={note} onChange={e=>onNote(e.target.value)} placeholder="Notes (optional)" rows={2} className="input-field text-xs"/><div className="flex gap-2"><button onClick={onPrint} className="btn-secondary flex-1 text-xs py-1.5">Print</button><button onClick={onApprove} disabled={busy} className="btn-success flex-1 text-xs py-1.5">{busy?"":" Approve"}</button><button onClick={onReject} disabled={busy} className="btn-danger flex-1 text-xs py-1.5">{busy?"":" Reject"}</button></div></div></div></div>);
}
