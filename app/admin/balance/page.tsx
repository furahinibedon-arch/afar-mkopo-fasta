...
export default function CompanyBalance(){
  ...
  const submit=async(e:React.FormEvent)=>{
    ...
    try{
      ...
      const r=await fetch(`${BASE}/api/admin/balance`,{method:"POST",headers:ah(),body:JSON.stringify({type:apiType,amount:parseFloat(form.amount),description:form.description,reference:form.type})});
      const d=await r.json();
      if(!r.ok)throw new Error(d.error);
      setMsg("Saved!");setForm({type:"CAPITAL",amount:"",description:""});load();
  ...
  const clearAll=async()=>{
    ...
    try{
      ...
      const d=await r.json();
      if(!r.ok)throw new Error(d.error);
      setConfirmClear(false);setEntries([]);
    }catch(ex:any){alert(ex.message||"Failed to clear");}
    finally{setClearing(false);}
  };
  const capital=entries.filter(e=>e.type==="CREDIT"&&(e.reference==="CAPITAL"||e.reference==="MANUAL")).reduce((s,e)=>s+Number(e.amount),0);
  const repaidIn=entries.filter(e=>e.type==="CREDIT"&&(String(e.reference||"").startsWith("REPAY_INSTALLMENT_")||String(e.reference||"").startsWith("LOAN_REPAY_"))).reduce((s,e)=>s+Number(e.amount),0);
  const disbursedOut=entries.filter(e=>e.type==="DEBIT"&&String(e.reference||"").startsWith("LOAN_DISBURSE_")).reduce((s,e)=>s+Number(e.amount),0);
  const otherOut=entries.filter(e=>e.type==="DEBIT"&&!String(e.reference||"").startsWith("LOAN_DISBURSE_")).reduce((s,e)=>s+Number(e.amount),0);
    ...
    <Layout portal="admin">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="page-subtitle">Capital, repayments, interest and disbursements.</p>
        </div>
        <button onClick={()=>setConfirmClear(true)} className="btn-danger flex items-center gap-2 shrink-0"><Trash2 className="w-4 h-4"/> Clear All</button>
      </div>
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-3 mb-3">
        <div className="kpi-card col-span-2 xl:col-span-1" style={{borderLeftColor:balance>=0?"#10b981":"#ef4444"}}>
          ...
          <div><p className="kpi-label">Current Balance</p>
          <p className={`kpi-value ${balance>=0?"text-emerald-700":"text-red-600"}`}>{balance>=0?"+":"-"}{fmt(balance)}</p>
          <p className="kpi-sub">{balance>=0?"Positive":"Negative"}</p></div>
        </div>
        <div className="kpi-card" style={{borderLeftColor:"#0284c7"}}>
          ...
          <div><p className="kpi-label">Capital Invested</p><p className="kpi-value text-sky-700">+{fmt(capital)}</p><p className="kpi-sub">Funds entered as capital</p></div>
        </div>
        <div className="kpi-card" style={{borderLeftColor:"#7c3aed"}}>
          ...
          <div><p className="kpi-label">Total Disbursed</p><p className="kpi-value text-violet-700">-{fmt(disbursedOut)}</p><p className="kpi-sub">Loans given out</p></div>
        </div>
      </div>
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-3 mb-6">
        <div className="kpi-card" style={{borderLeftColor:"#059669"}}>
          ...
          <div><p className="kpi-label">Customer Repayments</p><p className="kpi-value text-emerald-700">+{fmt(repaidIn)}</p><p className="kpi-sub">Cash received from borrowers</p></div>
        </div>
        <div className="kpi-card" style={{borderLeftColor:"#d97706"}}>
          ...
          <div><p className="kpi-label">Interest Earned</p><p className="kpi-value text-amber-700">+{fmt(interestEarned)}</p><p className="kpi-sub">Repaid minus principal</p></div>
        </div>
        <div className="kpi-card" style={{borderLeftColor:"#dc2626"}}>
          ...
          <div><p className="kpi-label">Other Expenses</p><p className="kpi-value text-red-600">-{fmt(otherOut)}</p><p className="kpi-sub">Non-loan outflows</p></div>
        </div>
      </div>
      <div className="card mb-6">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          ...
          <div><label className="label">Amount (Tsh)</label>
          <input type="number" required min={1} value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} className="input-field" placeholder="e.g. 500000"/></div>
          <div><label className="label">Description</label>
          <input value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} className="input-field" placeholder="e.g. Initial capital from director"/></div>
      ...
      <div className="card overflow-x-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          {entries.length>0&&(<button onClick={()=>setConfirmClear(true)} className="btn-danger text-xs py-1.5 px-3 flex items-center gap-1.5"><Trash2 className="w-3.5 h-3.5"/> Clear Ledger</button>)}
        </div>
        ...
        <table className="data-table">
          <thead><tr>{["Date & Time","Category","Amount","Description","Running Balance"].map(h=>(<th key={h}>{h}</th>))}</tr></thead>
          <tbody>
            {entries.length===0&&(<tr><td colSpan={5} className="text-center py-10 text-zinc-400">No entries yet.</td></tr>)}
            {entries.map((e:any,i:number)=>{
              ...
              return(<tr key={e.id}>
                ...
                <td><span className={`badge ${e.type==="CREDIT"?"bg-emerald-50 text-emerald-700 border border-emerald-200":"bg-red-50 text-red-700 border border-red-200"}`}>{entryLabel(e)}</span></td>
                <td className={`font-bold ${e.type==="CREDIT"?"text-emerald-600":"text-red-600"}`}>{e.type==="CREDIT"?"+":"-"}{fmt(Number(e.amount))}</td>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-slide-up">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center shrink-0"><AlertTriangle className="w-6 h-6 text-red-600"/></div>
              <div><h3 className="text-base font-bold text-zinc-900">Clear All Balance Records?</h3>
              <p className="text-sm text-zinc-500 mt-1">This will permanently delete all {entries.length} entries. This cannot be undone.</p></div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={clearAll} disabled={clearing} className="btn-danger flex-1 py-2.5">{clearing?"Clearing...":"Yes, Clear All"}</button>
