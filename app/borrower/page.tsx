...
export default function BorrowerPortal(){
  ...
  const sub=async(data:FD)=>{setBusy(true);setErr(null);try{await submitLoan({...data,dailyPayment:daily,interestRate:20,repaymentPeriod:30});setDone(true);}catch(e:any){setErr(e.message);}finally{setBusy(false);}}; 
  ...
  return(
    <Layout portal="borrower">
      ...
      <form onSubmit={handleSubmit(sub)}>
        ...
        <div className="flex items-center justify-between mt-6"><button type="button" onClick={prev} disabled={step===0} className="btn-secondary disabled:opacity-30"> Back</button><span className="text-xs text-slate-400 font-medium">Step {step+1} of {STEPS.length}</span>{step<STEPS.length-1?<button type="button" onClick={next} className="btn-primary">Next </button>:<button type="submit" disabled={busy} className="btn-primary">{busy?"Saving":"Submit"}</button>}</div>
      </form>
    </Layout>
  );
}
