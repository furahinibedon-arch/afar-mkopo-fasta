...
export default function StaffApplyPage() {
  ...
  useEffect(() => {
    ...
    if (role === "LOAN_OFFICER") { router.push("/staff/apply"); return; }
  }, [router]);
  ...
  const submit = async (data: FD) => {
    ...
    try {
      ...
      const result = await staffSubmitLoan({ ...data, borrowerPhone: data.borrowerPhone || ("admin_walkin_" + Date.now()), interestRate: rate, repaymentPeriod: rp, payment, repaymentType: data.repaymentType });
      setDone(result);
    } catch(e:any) { setErr(e.message); }
    ...
    <Layout portal="admin">
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <div className="flex gap-3 justify-center">
          ...
          <button onClick={() => router.push("/admin/loans")} className="btn-secondary">Back to Loans</button>
        </div>
      </div>
    </Layout>
  );
