...
w('app/admin/balance/page.tsx',`"use client";
...
export default function CompanyBalance(){
  ...
  return(<Layout portal="admin">
    <div className="mb-8"><h1 className="text-3xl font-black text-navy-800">Company Balance</h1><p className="text-slate-500 mt-1">Track money in and out of the business.</p></div>
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
      <div className={\`card border-l-4 \${balance>=0?"border-emerald-400":"border-red-400"}\`}><p className="label">Current Balance</p><p className={\`text-3xl font-black mt-1 \${balance>=0?"text-emerald-600":"text-red-600"}\`}>{fmt(balance)}</p><p className="text-xs text-slate-400 mt-1">{balance>=0?" Positive":" Negative"}</p></div>
      <div className="card border-l-4 border-blue-400"><p className="label">Total In</p><p className="text-3xl font-black text-blue-600 mt-1">{fmt(credits)}</p></div>
    ...
    <div className="card mb-8">
      <h2 className="text-lg font-black text-navy-800 mb-4">Add Entry</h2>
      ...
      <table className="w-full text-sm">
        ...
        {entries.map((e:any,i:number)=>{
          ...
          return(<tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50">
            <td className="py-3 px-3 text-slate-400 text-xs">{new Date(e.createdAt).toLocaleDateString()}</td>
            ...
            <td className={\`py-3 px-3 font-bold \${run>=0?"text-navy-800":"text-red-600"}\`}>{fmt(run)}</td>
          </tr>);
        })}</tbody>
      </table>}
    </div>
